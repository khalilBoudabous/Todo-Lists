...
function AdminTodoListDetailContent({ id }: { id: string }) {
    ...
    const fetchData = async () => {
      try {
        const [listRes, tasksRes] = await Promise.all([
          adminTodoListApi.getOne(todoListId),
          adminTodoListApi.getTasks(todoListId),
        ]);
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-muted mt-1">
            Owner: {todoList.user?.firstName} {todoList.user?.lastName} ({todoList.user?.email})
          </p>
        </div>
        <div className="flex gap-2">
          <Link
            href={`/dashboard/admin/todolists/${todoList.id}/edit`}
      ...
      <div className="bg-card-bg rounded-lg shadow mb-6">
        <div className="p-6 border-b border-border">
          <div className="flex justify-between items-center">
            ...
            <Link
              href={`/dashboard/admin/todolists/${todoList.id}/tasks/create`}
...
export default function AdminTodoListDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  return (
    <ProtectedRoute allowedRoles={['ROLE_ADMIN']}>
      <AdminTodoListDetailContent id={id} />
    </ProtectedRoute>
  );
}
